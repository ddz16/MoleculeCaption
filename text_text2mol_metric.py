...
parser.add_argument('--input_file', type=str, default='out.txt', help='path where test generations are saved')
...
parser.add_argument('--text_model', type=str, default='scibert/', help='Desired language model.')
...
with open('cids_to_smiles.pkl', 'rb') as f:
...
with torch.no_grad():
    for i, (smi, gt, out) in enumerate(outputs):
        ...
        if str(cid) not in mol2vec.keys():
            print(cid)
            continue
